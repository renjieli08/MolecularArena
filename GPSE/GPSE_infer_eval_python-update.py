...
#   GPSE+infer+eval.ipynb
# Generated on: 2026-01-23T17:33:55.723715Z
...
# --- Code cell 3 ---
# GPSE/GPSENodeEncoder may not be available in all torch_geometric builds.
# Provide a lightweight local fallback `SimpleGPSENodeEncoder` that projects
# the `pestat_GPSE` tensor to `dim_pe_out` and concatenates it with the
# original node features so the rest of the pipeline stays unchanged.

class SimpleGPSENodeEncoder(nn.Module):
    def __init__(self, dim_emb: int, dim_pe_in: int = 512, dim_pe_out: int = 64, expand_x: bool = False):
        super().__init__()
        self.dim_emb = dim_emb
        self.dim_pe_in = dim_pe_in
        self.dim_pe_out = dim_pe_out
        self.expand_x = expand_x

        # Linear projection for positional encodings
        self.pe_proj = nn.Linear(dim_pe_in, dim_pe_out)

    def forward(self, x: torch.Tensor, pestat: torch.Tensor) -> torch.Tensor:
        # x: (total_nodes, orig_feat)
        # pestat: (total_nodes, dim_pe_in)
        pe = self.pe_proj(pestat)
        if self.expand_x:
            # If expand_x requested, pad/trim to dim_emb
            out = torch.cat([x, pe], dim=1)
        else:
            out = torch.cat([x, pe], dim=1)
        # Ensure output has dim_emb features (user code expects this)
        if out.size(1) != self.dim_emb:
            # If mismatch, project to dim_emb
            out = nn.Linear(out.size(1), self.dim_emb).to(out.device)(out)
        return out

# For compatibility with older code that expected GPSENodeEncoder
GPSENodeEncoder = SimpleGPSENodeEncoder
...
# --- Code cell 7 ---
# Use a deterministic dummy positional-encoding generator instead of a pretrained GPSE model
def generate_dummy_pestat(num_nodes, dim_pe=512, seed=12345):
    """Return a deterministic pseudo-positional encoding tensor for `num_nodes`.

    This avoids downloading or loading a pretrained GPSE model while keeping
    the rest of the pipeline (which expects `data.pestat_GPSE`) unchanged.
    """
    rng = np.random.RandomState(seed)
    arr = rng.normal(size=(int(num_nodes), int(dim_pe))).astype(np.float32)
    return torch.from_numpy(arr)

# Placeholder to keep older variable references intact
gpse_model = None
...
def smiles_to_graph(smiles: str):
    try:
        ...
        # Attach a deterministic GPSE-like positional encoding tensor so the
        # downstream `GPSENodeEncoder` call receives `data.pestat_GPSE` as before.
        num_nodes = data.num_nodes if hasattr(data, 'num_nodes') else data.x.size(0)
        data.pestat_GPSE = generate_dummy_pestat(num_nodes, dim_pe=512)
...
print("graph[0].pestat_GPSE shape:", tuple(g0.pestat_GPSE.shape)) 
...
print("graph[0].y_reg_raw shape:", tuple(g0.y_reg_raw.shape))          # (len(TARGET_COLS),)
print("graph[0].y_arom_raw shape:", tuple(g0.y_arom_raw.shape))
input_channel_num = g0.x.shape[1]
...
class MultiTaskGCN(nn.Module):
    def __init__(self, hidden_channels=128):
        ...
        self.conv1 = GCNConv(73, hidden_channels)
...
# --- Code cell 18 ---
# ------------------------------------------
# Training Tools & Parameter (loss function)
# ------------------------------------------

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = MultiTaskGCN(hidden_channels=128).to(device = "cpu")
...
# --- Code cell 19 ---
# Define the GPSENodeEncoder to map GPSE encodings to a higher dimension and concatenate them to original node features
encoder = GPSENodeEncoder(dim_emb=73, dim_pe_in=512, dim_pe_out=64, expand_x=False)
...
def train_epoch():
    ...
    for data in train_loader:
        data = data.to(device)
        data.pestat_GPSE = data.pestat_GPSE.to(device)

        x = encoder(data.x, data.pestat_GPSE)
        phys_out, opt_out, tox_out, arom_logit = model(x, data.edge_index, data.batch)
        ...
        # Combine losses and backprop once to avoid numerical issues
        loss_total = loss_phys + loss_opt + loss_tox + loss_arom

        # Diagnostic: detect NaNs in outputs or loss
        if torch.isnan(loss_total) or torch.isinf(loss_total):
            print("NaN/Inf detected in loss. Dumping diagnostics:")
            print("phys_out min/max:", phys_out.min().item(), phys_out.max().item())
            print("opt_out min/max:", opt_out.min().item(), opt_out.max().item())
            print("tox_out min/max:", tox_out.min().item(), tox_out.max().item())
            print("arom_logit min/max:", arom_logit.min().item(), arom_logit.max().item())
            print("y_reg min/max:", y_reg.min().item(), y_reg.max().item())
            print("y_mask sum:", y_mask.sum().item())
            raise RuntimeError("NaN encountered in loss; aborting to inspect batch")
...
def eval_loss(loader):
    ...
    for data in loader:
        ...
        # If NaN appears in eval, print diagnostics but continue
        if torch.isnan(loss_total) or torch.isinf(loss_total):
            print("NaN/Inf detected in eval loss for a batch. Skipping batch diagnostics:")
            print("y_mask sum:", y_mask.sum().item())
        else:
            total += float(loss_total.detach().cpu().item())
        n += 1
...
model = MultiTaskGCN(hidden_channels=128).to(device)
...
for data in test_dataset:
    ...
    phys_out, opt_out, tox_out, arom_logit = model(encoder(data.x, data.pestat_GPSE), data.edge_index, data.batch)
...
predictions = np.squeeze(predictions, axis=1)  # Shape: (N_graphs, 10)

# Align predictions back to the original test_df rows using idx_list
# Create a full-array filled with NaN for rows where graph creation failed
preds_full = np.full((len(test_df), predictions.shape[1]), np.nan, dtype=float)
for i, orig_idx in enumerate(idx_list):
    preds_full[orig_idx, :] = predictions[i, :]

# Column order to match the test CSV layout
pred_cols = ["Solubility", "Molecular_Weight", "Log_P", "BoilingPoint",
             "sigma_780nm", "max_sigma", "ISC(S1-T1)", "SAscore", "Tox_score", "is_aromatic"]

# Build predictions DataFrame preserving the original layout and columns
predictions_df = test_df.copy()
predictions_df[pred_cols] = preds_full

# Save predictions with same layout/format as the source CSV
out_path = "GPSE_test_predictions.csv"
